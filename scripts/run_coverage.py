#!/usr/bin/env python3
"""Run the S&P 500 coverage engine.
...
local .env to run it for real on claude-sonnet-5.
    ...
    python scripts/run_coverage.py                        # full S&P 500 (real run: ~$9)
"""
...
from fund.coverage.universe import fetch_sp500  # noqa: E402
...
def main():
    ...
    print(f"Universe: {n} companies" + (f" (of {len(universe)} S&P 500)" if tickers else ""))
    if not client.mock:
